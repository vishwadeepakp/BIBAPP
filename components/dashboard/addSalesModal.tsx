...
function AutoCompleteDropdown({
  ...
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      try {
        const queryParam = `${searchType === 'product' ? 'productName' : 'brandName'}=${encodeURIComponent(debouncedQuery)}`
        const brandParam = brandName ? `&brandName=${encodeURIComponent(brandName)}` : ''

        const res = await api.get(`/stock/stock-suggestion?${queryParam}${brandParam}`, {
  ...
  }, [debouncedQuery, searchType, brandName])
        ...
        <div className="absolute z-50 mt-1 max-h-48 w-full overflow-y-auto rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-md">
          {suggestions.map((item, i) => (
            <div
              key={i}
              onClick={() => {
                const displayName = item.name || item.brand || ''
...
export function AddSalesModal({ open, onClose, onSave }: AddSalesModalProps) {
  ...
  const itemsTotalAfterDiscount = items.reduce((acc, item) => acc + (Number(item.total) || 0), 0)

  const overallDiscNum = Number(overallDiscountValue) || 0
  const globalDiscountAmount =
    overallDiscountType === 'percent'
      ? (itemsTotalAfterDiscount * overallDiscNum) / 100
      : overallDiscNum
